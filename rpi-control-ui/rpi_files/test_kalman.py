...
kalman.measurementNoiseCov = np.eye(2, dtype=np.float32) * 0.1 # Balances sensor vs. prediction
...
# PID (tuned)
# ─────────────────────────────────────────────
pan_pid  = PID(0.015, 0.001, 0.003, -6, 6)
tilt_pid = PID(0.010, 0.001, 0.002, -6, 6)
...
try:
    while True:
        ...
        if detection:
            ...
            # Clamp tilt early so we can check the exact new angle
            tilt_angle = max(TILT_MIN, min(TILT_MAX, tilt_angle))

            # Check the new state
            is_flipped = tilt_angle > 90

            # 1. WIPE PID MEMORY ON CROSSING
            # If we just crossed the 90-degree mark, wipe the pan PID to prevent a violent jerk
            ...
            if abs(error_x) > DEAD_ZONE:
                ...
                # 2. ZENITH SCALING (Gimbal Lock Fix)
                # Cosine of 90 degrees is 0. This smoothly scales down the pan speed 
                # as it approaches the exact vertical point, preventing violent stuttering.
                zenith_scale = abs(math.cos(math.radians(tilt_angle)))
                
                # Keep a minimum of 20% speed so it doesn't get completely stuck at exactly 90
                zenith_scale = max(0.2, zenith_scale) 
                pan_adj *= zenith_scale
